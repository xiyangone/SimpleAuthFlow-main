...
### 0. 新增手动模式体验修复与日志导出（2026/4/9）

- 手动模式下，自动运行会优先使用你在侧边栏填写的邮箱，不再误走 Burner 自动取邮箱旧逻辑
- 手动模式下隐藏“邮箱服务”和“验证码等待(秒)”配置，界面更聚焦手动输入场景
- 日志区新增 JSON 下载按钮，并与“清空”按钮放在同一组
- 错误提示默认只进入日志区，不再弹出错误 Toast 干扰操作

### 0.1 新增手动模式核心闭环（2026/4/9）
